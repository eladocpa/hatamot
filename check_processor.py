...
שיק שחזר מופיע ב-Maven בשתי תנועות:
  1. ההפקדה המקורית - תנועה חיובית (עם צילום השיק). *זו* שעליה
     אסור להוציא קבלה.
  2. פעולת ההחזרה - תנועה שלילית (גם עם צילום השיק, במינוס).

לכן אנחנו עובדים בשני מעברים:
  מעבר 1: קוראים את *כל* השיקים (חיוביים ושליליים) ומוציאים מספר שיק.
  מעבר 2: מספרי השיק שהופיעו בתנועה שלילית = "שיקים שחזרו".
          כל הפקדה חיובית עם מספר שיק כזה מסומנת "לא להוציא קבלה".
==================================================================
"""
...
def process_all(
    ...
    if bounced_check_numbers or bounced_amounts:
        print(f"\n  ⛔ זוהו שיקים שחזרו: "
              ...
              f"{len(bounced_amounts)} סכומים שליליים.")
    ...
    for idx, (item, details) in enumerate(zip(items, read_cache), start=1):
        ...
        # התנועה השלילית עצמה = פעולת ההחזרה. מציגים לתיעוד,
